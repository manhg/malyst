...
namespace UI
{    
    public partial class FInterpretG2 : Form,Core.IInterpret
    {
        class MarkRowInfo
        {
            /// <summary>
        ...
        private void FInterpretG2_Load(object sender, EventArgs e)
        {
        ...
        {
            //if (((Core.CoreMsgCapable)sender).IsForFInterpretG2)
        ...
        private void gridMark_Populate(Core.Discover discover, int index, string fileName)
        {
            ...
            row.Tag = new MarkRowInfo();
            ((MarkRowInfo)row.Tag).FileName = fileName;
            ((MarkRowInfo)row.Tag).GridIndex = index;
            // Thêm
            ((MarkRowInfo)row.Tag).ResultIndex = gridMark.Rows.Add(row);
        ...
        private void Print()
        {
            ...
            prDoc.DocumentName = "Kết quả chấm điểm từ Malyst";
